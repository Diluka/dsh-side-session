 ...
 *      never be reopened.
 *
 * The client half calls the Remote namespace `sideSession` (`ctx.remote.sideSession`).
 */
...
/** The Remote service exposing side-session operations to the client half. */
class SideSessionService extends TypertRemoteService {
  constructor(ctx) {
    super(ctx, 'sideSession')
...
/**
 * Simulate the stage-3 `@Remote(name)` method decorator in plain JS: the
 * decorator only registers an initializer, which Typert invokes per
 * instance — call it with the concrete instance so the marker lands on the
 * service prototype.
 */
function markRemote(service, method, exportName) {
  ...
  Remote(exportName)(proto[method], context)
}

export function apply(ctx) {
  const service = new SideSessionService(ctx)
  markRemote(service, 'create', 'create')
  markRemote(service, 'prompt', 'prompt')
  markRemote(service, 'cancel', 'cancel')
  markRemote(service, 'close', 'close')
  markRemote(service, 'list', 'list')
  markRemote(service, 'status', 'status')
}
