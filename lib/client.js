...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    exports.apply = (ctx) => {
       ...
       *
       *  Persisted to sessionStorage so an interrupted render (slot crash /
       *  plugin hot-reload) can never lose the session binding — without this,
       *  a second send after a remount would fork a brand-new session. */
      const SIDE_KEY = 'dsh-side-session.state'
      let sideState = null
      try {
        const raw = window.sessionStorage.getItem(SIDE_KEY)
        if (raw !== null) {
          const parsed = JSON.parse(raw)
          if (parsed !== null && typeof parsed === 'object' && typeof parsed.mainId === 'string') sideState = parsed
        }
      ...
      const setSide = (state) => {
        sideState = state
        try {
          if (state === null) window.sessionStorage.removeItem(SIDE_KEY)
          else window.sessionStorage.setItem(SIDE_KEY, JSON.stringify(state))
        } catch { /* storage unavailable */ }
      ...
      function SidePanel() {
        ...
        /** Pending input queue rows (from host status): { id, text, placement }. */
        const [queue, setQueue] = React.useState([])

        // Poll the host for the pending input queue while a session exists
        // (main-session queue dock data source).
        React.useEffect(() => {
          ...
          const tick = async () => {
            if (stopped) return
            try {
              const res = await sideRemote.status({ sessionId })
              if (res !== null && res.ok === true && Array.isArray(res.queue)) {
                setQueue(res.queue)
              }
            } catch { /* transient; next tick */ }
            if (!stopped) timer = ctx.timeout(tick, 3000)
          }
          tick()
          return () => {
            stopped = true
        ...
        const steerRow = async (row) => {
          if (sessionId === null || !running) return
          try {
            await sideRemote.steer({ sessionId, queueId: row.id })
          } catch (error) {
        ...
        return React.createElement(
          ...
          React.createElement(
            ...
            queue.length > 0
              ? React.createElement(
                  'div',
                  { className: 'ss-queue-dock' },
                  React.createElement(
                    'div',
                    { className: 'ss-queue-panel' },
                    queue.map((row) =>
                      React.createElement(
                        'div',
                        { key: row.id, className: 'ss-queue-row' },
                        React.createElement(IconQueueOutline14, { className: 'ss-queue-lead' }),
                        React.createElement('span', { className: 'ss-queue-text' }, row.text),
                        React.createElement(
                          'div',
                          { className: 'ss-queue-actions' },
                          React.createElement(
                            'button',
                            {
                              type: 'button',
                              className: 'ss-queue-action',
                              'aria-label': '插话（引导到当前步骤）',
                              title: running ? '插话（引导到当前步骤）' : '仅在运行时可用',
                              disabled: !running,
                              onClick: () => steerRow(row)
                            },
                            React.createElement(IconSendOutline14, null)
                          )
                        )
                      )
                    )
