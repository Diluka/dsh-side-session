        ...
        return React.createElement(
          ...
          React.createElement(
            ...
            running
              ? React.createElement(
                  Button,
                  { variant: 'ghost', size: 'sm', icon: React.createElement(IconStopFill16, null), className: 'ss-header-btn', onClick: () => { sideRemote.cancel({ sessionId }).catch((error) => console.error('side-session cancel failed:', error)) }, title: '停止' }
                )
              : null,
            React.createElement(
              Button,
              { variant: 'ghost', size: 'sm', icon: React.createElement(IconCloseOutline16, null), className: 'ss-header-btn', onClick: close, title: '关闭并销毁侧边会话' }
            )
          ),
          React.createElement(
            'div',
              ...
              React.createElement(
                ...
                React.createElement(
                  'button',
                  {
                    ...
                    className: 'ss-send',
                    onClick: send,
                    disabled: sending || draft.trim() === '' || (session === null && initialized),
                    title: sending ? '停止生成' : '发送',
                    'aria-label': sending ? 'Stop generating' : 'Send'
                  },
                  React.createElement(sending ? IconStopFill16 : IconSendOutline16, null)
                )
              )
            )
          )
...
/* ---- tool calls: primitives Pill chips (main-session pill language) ---- */
