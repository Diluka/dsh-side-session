    ...
    exports.apply = (ctx) => {
      ...
      function ToolResultRow({ node }) {
        ...
        return React.createElement(
          ...
          React.createElement(
            'div',
            { className: 'ss-tool-chip ' + (node.isError ? 'error' : '') },
            React.createElement('span', { className: 'ss-tool-name' }, name)
          ),
          resultText !== ''
        ...
        return React.createElement(
          ...
          React.createElement(
            ...
            running
              ? React.createElement(
                  'button',
                  { className: 'ss-header-btn', onClick: () => { sideRemote.cancel({ sessionId }).catch((error) => console.error('side-session cancel failed:', error)) }, title: '停止' },
                  '■'
                )
              : null,
            React.createElement('button', { className: 'ss-header-btn', onClick: close, title: '关闭并销毁侧边会话' }, '✕')
          ),
          React.createElement(
            'div',
...
.side-session-header .ss-status {
  width: 8px; height: 8px; border-radius: 50%;
  background: var(--dsw-alias-label-tertiary, #9aa0a6); flex: none;
}
.side-session-header .ss-status.running { background: #34a853; animation: ss-pulse 1.2s ease-in-out infinite; }
@keyframes ss-pulse { 50% { opacity: .4; } }
.side-session-header .ss-header-btn {
  border: none; background: transparent; color: var(--dsw-alias-label-secondary, #61666b);
  border-radius: 50%; width: 28px; height: 28px; flex: none;
  display: inline-flex; align-items: center; justify-content: center;
  cursor: pointer; font-size: 13px; line-height: 1;
}
.side-session-header .ss-header-btn:hover { background: var(--dsw-alias-interactive-bg-hover, rgba(0,0,0,.06)); }
.side-session-header .ss-header-btn:disabled { opacity: .45; cursor: default; }
.side-session-body {
...
/* ---- reasoning fold (main-session Think look) ---- */
.ss-reasoning { width: 100%; border: none; margin: 0; }
.ss-reasoning summary {
...
.ss-reasoning summary::-webkit-details-marker { display: none; }
.ss-reasoning summary::before { content: '▸'; transition: transform .15s ease; font-size: 11px; }
.ss-reasoning[open] summary::before { transform: rotate(90deg); }
.ss-reasoning summary:hover { background: var(--dsw-alias-interactive-bg-hover, rgba(0,0,0,.05)); }
...
/* ---- tool calls: pill chips (main-session callRow language) ---- */
.ss-tools { display: flex; flex-wrap: wrap; gap: 6px; }
.ss-tool-chip {
  display: inline-flex; align-items: center; gap: 6px;
  font-size: 13px;
...
.ss-tool-chip .ss-tool-name { color: var(--dsw-alias-label-primary, #0f1115); font-weight: 500; }
