  ...
  factory: (require) => {
    ...
    exports.apply = (ctx) => {
      ...
      const sideRemote = {
        ...
        steer: (args) => apiCall('steer', args)
      }

      /** Imperatively fill the queue dock rows. Deliberately DOM-only: a
       *  React-rendered conditional branch crashes the render (shell runtime
       *  TDZ on first mount of that branch), so the container is a plain
       *  always-rendered div and rows are written straight into it. */
      function renderDockDOM(rows, runningNow) {
        const dock = document.querySelector('.side-session-overlay .ss-queue-dock')
        if (dock === null) return
        while (dock.firstChild !== null) dock.removeChild(dock.firstChild)
        if (!Array.isArray(rows) || rows.length === 0) return
        const panel = document.createElement('div')
        panel.className = 'ss-queue-panel'
        for (const row of rows) {
          const el = document.createElement('div')
          el.className = 'ss-queue-row'
          const lead = document.createElement('span')
          lead.className = 'ss-queue-lead'
          lead.textContent = '›'
          const text = document.createElement('span')
          text.className = 'ss-queue-text'
          text.textContent = row.text
          const actions = document.createElement('div')
          actions.className = 'ss-queue-actions'
          const btn = document.createElement('button')
          btn.type = 'button'
          btn.className = 'ss-queue-action'
          btn.textContent = '⟶'
          btn.setAttribute('aria-label', '插话（引导到当前步骤）')
          btn.disabled = !runningNow
          btn.title = runningNow ? '插话（引导到当前步骤）' : '仅在运行时可用'
          btn.addEventListener('click', () => {
            sideRemote.steer({ sessionId: getSide()?.sessionId, queueId: row.id }).catch((error) => {
              console.error('side-session steer failed:', error)
            })
          })
          actions.append(btn)
          el.append(lead, text, actions)
          panel.append(el)
        }
        dock.append(panel)
      }

      // Plugin-level queue polling: independent of any component render, so
      // a crashed/interrupted panel render can never stop the queue dock.
      // (Client ctx has no timer service — use the browser timer directly.)
      let dockTimer = null
      const dockTick = async () => {
        const side = getSide()
        if (side !== null && typeof side.sessionId === 'string') {
          try {
            const res = await sideRemote.status({ sessionId: side.sessionId })
            if (res !== null && res.ok === true && Array.isArray(res.queue)) {
              renderDockDOM(res.queue, res.status === 'running')
            }
          } catch { /* transient; next tick */ }
        }
        dockTimer = setTimeout(dockTick, 2000)
      }
      dockTimer = setTimeout(dockTick, 800)
      ctx.effect(() => () => {
        if (dockTimer !== null) clearTimeout(dockTimer)
      }, 'dsh-side-session: queue dock polling')
      ...
      function AssistantMessage({ blocks, running, toolResults = [] }) {
          ...
          tools.length > 0
            ? React.createElement(
                ...
                React.createElement(
                  ...
                  tools.map((t, idx) => {
                    // Pair each tool call with its own result BY NAME, so a
                    // failed/cancelled call (no result event) cannot steal the
                    // next tool's output.
                    const resultIndex = toolResults.findIndex((r) => r.name === t.name)
                    const result = resultIndex >= 0 ? toolResults[resultIndex].text : ''
                    return React.createElement(
                      'div',
                      { key: idx, className: 'ss-tool-item' },
                      React.createElement(
                        Pill,
                        { className: 'ss-tool-chip' },
                        React.createElement('span', { className: 'ss-tool-name' }, t.name)
                      ),
                      result !== ''
                        ? React.createElement('div', { className: 'ss-tool-result' }, result)
                        : null
                    )
                  })
                )
      ...
      function SidePanel() {
        ...
        /** Live mirror of `sessionId` for async handlers: a re-render can lag
         *  behind `create` (external-store notification is async), so sends
         *  must not read the render-time closure value. */
        const sessionIdRef = React.useRef(null)
        sessionIdRef.current = sessionId
        /** Locally optimistic user messages: shown instantly on send, removed
         *  once the same message flows back through the session snapshot
         *  (main-session pending-steering behavior). While the agent is busy
         *  they double as the queue indicator. */
        const [pendingMsgs, setPendingMsgs] = React.useState([])


        /** Steer one queued row into the running turn (插话). */
        ...
        // Drop optimistic messages once the same text flows back through the
        // session snapshot (matched by text among THIS session's own user
        // messages — forked history and the boundary row are skipped, so a
        // repeat of an old question cannot echo a pending row).
        React.useEffect(() => {
          ...
          for (const node of ns) {
            ...
            if (typeof node.seq === 'number' && seedLength !== null && node.seq < seedLength) continue
            const src = node?.source
            if (src !== null && typeof src === 'object' && src.kind === 'plugin' && src.plugin === 'side-session') continue
            const text = textOfContent(node.content)
            if (text === '') continue
            const match = pendingMsgs.find((p) => p.text === text && !echoed.has(p.id))
            if (match !== undefined) echoed.add(match.id)
          }
          if (echoed.size > 0) setPendingMsgs((list) => list.filter((p) => !echoed.has(p.id)))
        }, [snapshot, pendingMsgs, seedLength])
        ...
        /** Cancel the in-flight generation (send button, no draft). */
        const cancelNow = () => {
          const id = getSide()?.sessionId ?? null
          if (id !== null) {
            sideRemote.cancel({ sessionId: id }).catch((error) => console.error('side-session cancel failed:', error))
          }
          setSending(false)
        }
        ...
        const send = async () => {
          ...
          if (text === '') return
          setSending(true)
          // Optimistic render: the user's message appears instantly; the
          // snapshot echo (matched by text + time) removes it later.
          const pendingId = 'p' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6)
          setPendingMsgs((list) => [...list, { id: pendingId, text, time: Date.now() }])
          try {
            // First message: initialize the side session (fork seed + recent
            // context + agent creation) now, then deliver the prompt.
            // Read the live side state (plugin-scoped, survives remounts) —
            // never the render closure value.
            let target = getSide()?.sessionId ?? null
            if (target === null) {
        ...
        const rows = []
        /** Tool outputs accumulated from standalone tool/result events since
         *  the last assistant message; merged into that message's tool fold. */
        let toolResults = []
        ...
        for (let index = 0; index < visible.length; index++) {
          ...
          if (node.kind === 'user') {
            ...
            // Tool calls live inside the assistant message's blocks; the
            // standalone tool/result events that follow carry the outputs.
            // Merge them in order so one tool shows exactly once (folded).
            const results = toolResults.splice(0)
            rows.push(React.createElement(AssistantMessage, { key: node.seq ?? `a${index}`, blocks: node.blocks, running, toolResults: results }))
          } else if (node.kind === 'tool-result') {
            const nodeName = node.call?.name ?? node.callId ?? ''
            toolResults.push({ name: nodeName, text: textOfContent(node.content).slice(0, 600) })
        ...
        // Optimistic pending messages appended at the end of the flow with a
        // queue hint while the agent is busy (main-session steering behavior).
        const pendingRows = pendingMsgs.map((p, index) => {
          const d = new Date(p.time)
          const hhmm = `${String(d.getHours()).padStart(2, '0')}:${String(d.getMinutes()).padStart(2, '0')}`
          return React.createElement(
            'div',
            { key: p.id, className: 'ss-row user' },
            React.createElement(
              'div',
              { className: 'ss-user-stack' },
              React.createElement('div', { className: 'ss-bubble' }, p.text),
              React.createElement(
                'div',
                { className: 'ss-meta' },
                `${hhmm} · ${running || sending ? '排队中…' : '已发送'}`
              )
            )
          )
        })
        ...
        return React.createElement(
          ...
          React.createElement(
            ...
            rows.length === 0 && pendingRows.length === 0
              ? React.createElement(
                  ...
                  React.createElement(
                    'div',
                    { className: 'ss-empty-headline' },
                    React.createElement(FishLogo, { size: 34, className: 'ss-empty-fish' }),
                    React.createElement('span', { className: 'ss-empty-title' }, '侧边临时会话'),
                    React.createElement('span', { className: 'ss-empty-preview' }, 'Preview')
                  ),
                  React.createElement(
                    'div',
              ...
              : rows,
            ...pendingRows,
            running && (lastIsUser || pendingMsgs.length > 0)
              ? React.createElement(
                  'div',
          ...
          React.createElement(
            ...
            React.createElement('div', { className: 'ss-queue-dock' }),
            React.createElement(
              'div',
              { className: 'ss-composer-card' },
              React.createElement('textarea', {
                value: draft,
                placeholder: sessionId === null ? '描述你想了解什么…' : '输入消息…（Enter 发送）',
              ...
              React.createElement(
                ...
                React.createElement(
                  'button',
                  {
                    type: 'button',
                    className: 'ss-send',
                    onClick: () => { if (hasDraft) send(); else if (sending || running) cancelNow() },
                    disabled: !hasDraft && !sending && !running,
                    title: hasDraft ? '发送（运行中排队）' : sending || running ? '停止生成' : '发送',
                    'aria-label': hasDraft ? 'Send message' : sending || running ? 'Stop generating' : 'Send'
                  },
                  React.createElement(hasDraft || (!sending && !running) ? IconSendOutline16 : IconStopFill16, null)
                )
              )
            )
          )
...
.side-session-body .ss-empty-fish {
  grid-area: 1/1;
  color: var(--dsw-alias-label-primary, #0f1115);
}
.side-session-body .ss-empty-title { grid-area: 1/2; }
.side-session-body .ss-empty-preview {
  grid-area: 1/3;
  align-self: start; margin-top: 2px; margin-left: -3px;
  border: 1px solid var(--dsw-alias-interactive-bg-hover, rgba(0,0,0,.06));
  background: var(--dsw-alias-state-business-tertiary, #e8eefc);
  color: var(--dsw-alias-label-primary-bluish, #2563eb);
...
.ss-tools .ss-tools-detail { display: flex; flex-direction: column; gap: 8px; margin-top: 6px; padding: 0 2px; }
.ss-tool-item { display: flex; flex-direction: column; gap: 4px; }
.ss-tool-item .ss-tool-chip { align-self: flex-start; }
...
/* ---- pending queue dock above the composer (main-session QueueDock look) ---- */
.ss-queue-dock {
  flex: none;
  margin: 0 auto;
  width: 100%;
...
.ss-queue-panel {
  background: var(--dsw-specific-tip, #f0f3fa);
...
.ss-queue-row:hover { background: var(--dsw-alias-interactive-bg-hover, rgba(0,0,0,.05)); }
.ss-queue-lead { flex: none; color: var(--dsw-alias-label-tertiary, #81858c); }
.ss-queue-text {
  flex: 1;
  min-width: 0;
  overflow: hidden;
...
.ss-queue-actions { flex: none; display: flex; gap: 4px; }
.ss-queue-action {
  border: none;
...
.ss-queue-action:hover:not(:disabled) { background: var(--dsw-alias-interactive-bg-hover, rgba(0,0,0,.08)); color: var(--dsw-alias-label-primary, #0f1115); }
.ss-queue-action:disabled { opacity: .35; cursor: default; }
.ss-composer-card {
