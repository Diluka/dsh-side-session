 ...
 *  <system-reminder> blocks (skill catalogs, runtime context snapshots). */
function isSystemNoise(text) {
  return typeof text === 'string' && (text.includes('<system-reminder>') || text.includes('<system_reminder>'))
}
...
function recentMainContext(parent, fromSeq = 0, maxChars = 6000) {
  ...
  for (const e of parent.session.events) {
    ...
    if (e.type === 'user/message') {
      const data = e.data
      // Drop system-injected noise (skill catalogs, runtime reminders, …):
