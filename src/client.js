 ...
 *     main session (completed turns only, as reference context) and returns
 *     the child session id plus the seed boundary;
 *   - the panel binds that session through the standard `sessions` service,
 *     renders the message flow (markdown, reasoning folds, tool chips) and
 *     sends prompts through the host half (`side-session/prompt`);
 *   - closing the panel calls host `side-session/close`, which disposes the
 *     agent, removes its persisted artifact, and can never be reopened.
 *
...
.ss-md-p:last-child { margin-bottom: 0; }
...
.ss-md-code {
  ...
  font-size: 12px;
  background: var(--dsw-alias-bg-base, #141418);
...
.ss-md-ul li, .ss-md-ol li { margin: 2px 0; }
.ss-md-quote {
  border-left: 3px solid var(--dsw-alias-border-l2, rgba(128,128,128,.4));
  padding: 2px 10px;
  margin: 6px 0;
  opacity: .85;
}
...
.ss-md-table th { background: rgba(128,128,128,.12); }
.ss-assistant a { color: var(--dsw-alias-brand-primary, #4d93f8); }
/* --- reasoning fold --- */
.ss-reasoning {
  border-left: 2px solid var(--dsw-alias-border-l2, rgba(128,128,128,.35));
  margin: 4px 0 8px;
  font-size: 12px;
  opacity: .72;
}
...
.ss-tool-chip {
  ...
  border-radius: 999px;
  padding: 1px 8px;
  color: var(--dsw-alias-label-primary-dimmed, #b0b4bb);
}
.ss-tool-chip.error { color: #ff8080; border-color: rgba(255,80,80,.4); }
.ss-tool-result {
  align-self: stretch;
  font-size: 11px;
  font-family: var(--dsw-font-family-code, ui-monospace, monospace);
...
.side-session-footer .ss-send:hover { background: var(--dsw-alias-button-primary-hover, #3d7fd8); }
.side-session-footer .ss-send:disabled { opacity: .45; }
`)

    /** Render one inline-markdown text into React elements (code/bold/italic/link). */
    ...
    /** Render a markdown block list into React elements. */
    ...
    /** One assistant message: reasoning fold + markdown body + tool chips. */
    ...
    function SidePanel() {
      ...
      // everything else after the seed boundary is the side session's own turn.
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          running
            ? React.createElement(
                'button',
                { onClick: () => { host.call('side-session/cancel', { sessionId }).catch((error) => console.error('side-session cancel failed:', error)) }, title: '停止' },
                '停止'
              )
            : null,
